...
During the study of the Curry-Howard isomorphism we start having the identification between logic and lamdba-calculus. One important note is the fact that, as we can consider different logic deduction system, we can consider different typings, each one paired with a deduction system. When restricted to the world of categories these gave us also another set of paired categories.\\

In this fashion we can go in the other direction: having laid the intuition of what is a natural number in category theory we can proceed to explain what a natural number is in Typed Lambda calculus. Relating the notions of typed lambda calculus shown in \cite[Section 10]{lambek1985cartesian}, \cite[Section 6]{selinger2008lecture} and \cite[Section 10]{cardone2006history}, we can see that it is opted by Lambek to include to simply include some structure over the basic type $\iota$ to provide with a natural number type:

\begin{definition}
  \emph{Natural number simply type lambda-calculus} is a simply type lamdba calculus where:
  \begin{enumerate}
  \item
  \item
  \item 
...
This option has some positive notes{\color{red}Extend}:
\begin{enumerate}
\item We enjoy that the natural numbers are a basic contruction, we were already assuming estructure.
\item We have, as in categories, an object $N$ that is a natural number.
\item It is a hugely retalable with our future contection with category theory.
\end{enumerate}
...
\begin{definition}[Church numerals in typed lamdba-calculi]
  We define the $n$th Church numeral in typed lambda calculi as:
  $$\overline n = \lambda f^{\iota \to \iora}. \lambda x^{\iota}. f^{n}(\iota).$$
\end{definition}

Then we have a formal definition.

The study of this area is strangely obscure. 
