...
\item First of all, as every product exists, and it specified by $F_2$, we can define $F_3^b$ for every $b$. Then, it adjointness states that:
  ...
  This object $d^b$ is called the exponential object or map object. To define this adjoint, we have defined a functor $F_3^b$ on both functor and arrows ($F_3^b(f:a\to a') = f\times 1_b: a\times b\to a'\times b$), but the functor $G_3^b$ is defined only on the objects. To specify the adjoint we need to provide an arrow $\varepsilon$:
