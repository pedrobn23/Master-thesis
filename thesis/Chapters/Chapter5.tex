...
\begin{definition}[Interpretation of a Martin-L\"of theory]
  ...
  \begin{itemize}
  \item a type depending on a free variable $x^a$ will be interpreted as an object $\chi: \overline{x} \to $in $C/\overline{a}$, for a suitable $\overline{a}$.
  \item terms will be interpreted as morphisms in the appropriate slice categories.
  \end{itemize}
  In particular, a closed type $t$, will correspond to an object $\overline{t}\in C$.
\end{defintion}
\begin{remark}
  Lets analyze the considerations that are held whenever considering a type $t_{n+1}(x_1^{t_1},...,x_n^{t_n})$ that satisfy the condition on variables. $t_1$, being closed, correspond to an object $\overline t_1\in C$. Then we proceed to consider $t_2(x_1^{t_1})$, then $\overline t_2$ is a morphism $\chi_2:\overline t_2\to \overline t_1$. Then we type $t_{3}(x_1^{t_1}, x_2^{t_2})$, is maps to $\chi_3: \overline{t_3}\to \overline{t_2}$. This also induced a morphism $\chi_3' = \langle \chi_2\chi_3, \chi_3\rangle$.
\end{remark}


\begin{definition}[Interpretation of a Martin-L\"of theory - second part]
