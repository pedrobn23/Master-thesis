...
\begin{itemize}
\item Bifunctors
\item The $Hom(\cdot,\cdot)$ bifunctor
\end{itemize}

\subsubsection{Functor Categories}

We continue defining functor categories, that is, categories where we consider the functors as objects and natural transformation as arrows in some sense. This concept will be instrumental in further consideration in the realm of functional programming (in particular, in the definition of monad). Lets define the composition of two natural transformation.

\begin{definition}
Let $C$ and $B$ be two categories, $R,S,T : C \to B$ be functors, and let $\tau: R \to S$, $\sigma:S\to R$, we define the composition $(\tau \circ \sigma)c = \tau c\circ \sigma c$.
\end{definition}
...
This composition of natural transformation is called \emph{vertical composition}, in opposition to the \emph{horizontal composition}\ref{hortizontal-composition}
t
